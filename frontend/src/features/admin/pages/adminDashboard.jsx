...
import SuperAdminManagement from "../components/SuperAdminManagement"; // ✅ new component
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#29323C] to-[#485563] text-white">
      ...
      <div className="flex flex-1 px-6 py-4 gap-6">
        {/* Sidebar with role-based options */}
        ...
        <main className="flex-1 bg-black/20 rounded-2xl p-6 shadow-lg overflow-y-auto">
          ...
          {/* ✅ Superadmin-only */}
          {role === "superadmin" && activeTab === "codes" && <CodeGenerator />}
          {role === "superadmin" && activeTab === "users" && <UserManagement />}
          {role === "superadmin" && activeTab === "admins" && <SuperAdminManagement />}
        </main>
      </div>
    </div>
  );
